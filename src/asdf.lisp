...
;; This is taken from paren-files (c) Red Daly <reddaly@gmail.com>

(in-package :serve.paren.asdf)

(defsection @asdf-manual (:title "Define paren source files in asdf system definition")
...
(defmethod perform ((op asdf::parenscript-compile-op) (file asdf::javascript-file))
  ...
  (write-string (slurp-file (component-pathname file)) (output-stream op)))
...
(defun compile-ps-system (system)
